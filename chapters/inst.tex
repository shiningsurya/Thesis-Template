...
\par Writer and Process Baseband have been developed, written by Dr. Matthew Kerr. The author has written Coadder, Trigger mechanics, Trigger Hook and TriggerMaster.
...
\par \mpi~also offers capability of tuning by the Multi Component Architecture (MCA). MCA helps you to configure the various runtime parameters of \mpi jobs from pre-defined options to increase the performance. The tuning options used are described in~\autoref{ssub:reduce}.
...
\begin{figure}
	...
	\includegraphics[width=0.8\textwidth,keepaspectratio]{Reduce_Raben.png}
...
A linear trend is evident in the \textt{reduce\_coadd}. This issue caused multiple pipeline failures around $6^h$ mark when the lag in the step gets too high and pipeline cannot function realtime.
...
For a voltage trigger, it outputs a \textt{metar} file as described in~\autoref{ssub:metar}, and~\autoref{tab:metar}.
...
\begin{table}[]
	...
	\begin{tabular}{lll}
		\toprule
		\begin{tabular}[c]{@{}l@{}}Type of\\ data\end{tabular} & \begin{tabular}[c]{@{}l@{}}Number of\\ fields\end{tabular} & Data                                                                                                                 \\ \midrule
...
\begin{figure}
	\centering
	\label{fig:tp}
	\includegraphics[width=\textwidth,keepaspectratio]{tplot.png}
...
\begin{table}[]
	...
	\begin{tabular}{lll} \toprule
		...
		\multirow{8}{*}{Delays}  & VLITE\_ANT\_ID & VLITE Antenna ID                                                                                                       \\
														 ...
														 & ENABLE         & If antenna is added to the array                                                                                       \\ \midrule
...
\par There are two pipeline codebases. Writer and process-baseband wholly contributed by Dr. Matthew Kerr are found in \textt{vlite-fast} (\url{https://github.com/kerrm/vlite-fast}).
...
\par \texttt{asgard} is the pipeline code written by me which makes \vf possible. It is written in \textt{C++}, \textt{Python} and \textt{BASH}. The breakdown of the lines of code (LOC) written are given in~\autoref{tab:codebreak}.
